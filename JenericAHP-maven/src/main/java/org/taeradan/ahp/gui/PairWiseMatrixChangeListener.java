...
public class PairWiseMatrixChangeListener
	...
	public void tableChanged(TableModelEvent evt) {
		Logger.getAnonymousLogger().log(Level.INFO, "row={0},column{1}", new Object[]{evt.
					getFirstRow(),
																					  evt.
					getColumn()});
		if (evt.getFirstRow() >= evt.getColumn()) {
			final PairWiseMatrixTableModel prefMatrix = (PairWiseMatrixTableModel) evt.
					getSource();
			Double value = null;
			final Object nonParsedValue = prefMatrix.getValueAt(
					evt.getFirstRow(), evt.getColumn());
			Logger.getAnonymousLogger().log(Level.INFO, "Non parsed value = {0}",
											nonParsedValue);
...
//			If the changed value is a Double, just write it
			...
			Logger.getAnonymousLogger().log(Level.INFO, "Parsed value = {0}", value.
					doubleValue());
			...
			if (value == 0) {
				String newValue =
					   JOptionPane.showInputDialog(
						"The value \"0\" is not allowed here\nEnter a new value :");
				if (newValue == null) {
					newValue = "1";
				}
...
//			Case where the value is not entered : DON'T WORK FOR NOW
			...
			prefMatrix.setValueAt(( 1 / value ), evt.getColumn(), evt.
					getFirstRow());
		}
	}
}
